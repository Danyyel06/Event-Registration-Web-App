...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title) || empty($date) || empty($venue) || $capacity < 1) {
        ...
        $stmt = $pdo->prepare('UPDATE events SET title=:title,description=:description,date=:date,venue=:venue,capacity=:capacity WHERE id=:id');
        $stmt->execute([':title'=>$title,':description'=>$description,':date'=>$date,':venue'=>$venue,':capacity'=>$capacity,':id'=>$id]);
...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .page-sub { font-size:.83rem;color:var(--ink-muted);margin-top:.4rem; }
        ...
        .field input,.field textarea { width:100%;padding:.625rem .875rem;border:1px solid var(--border);border-radius:6px;font-size:.875rem;font-family:var(--font);color:var(--ink);background:var(--surface);outline:none;transition:border-color .15s,box-shadow .15s; }
        .field textarea { min-height:90px;resize:vertical; }
        .field input:focus,.field textarea:focus { border-color:var(--black);box-shadow:0 0 0 2px rgba(10,10,10,.08); }
        ...
        @media(max-width:540px){ .form-row{grid-template-columns:1fr} .wrap{padding:1.75rem 1.25rem 4rem} .form-body{padding:1.25rem 1.25rem 0} .divider{margin:1.25rem -1.25rem;padding:.75rem 1.25rem} .form-footer{padding:1rem 1.25rem} }
...
<div class="wrap">
    <div class="page-header">
        ...
        <p class="page-sub"><?= htmlspecialchars($event['title']) ?></p>
    </div>
            ...
            <div class="form-body">
                ...
                <div class="form-row">
                    ...
                    <div class="field">
                        ...
                        <input type="number" id="capacity" name="capacity" min="1" value="<?= (int)$event['capacity'] ?>" required>
                    </div>
                </div>
                <div class="field">
